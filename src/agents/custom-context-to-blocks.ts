...
const HISTORICAL_TOOL_CALL_PREFIX = "[Historical tool call:";
...
export function promoteHistoricalContextToBlocks(message: AssistantMessage): void {
  ...
  const hasMarker = message.content.some((b) => {
    ...
    return r.type === "text" && typeof r.text === "string" && r.text.includes(HC_PREFIX);
  });
  if (!hasMarker) {
    return;
  }
...
type ParsedHistoricalToolCall = {
  toolName: string;
  args: Record<string, unknown>;
};

function parseHistoricalToolCallThinking(text: string): ParsedHistoricalToolCall | null {
  const trimmed = text.trim();
  if (!trimmed.startsWith(HISTORICAL_TOOL_CALL_PREFIX)) {
    return null;
  }

  const nameClose = trimmed.indexOf("]", HISTORICAL_TOOL_CALL_PREFIX.length);
  if (nameClose < 0) {
    return null;
  }
  const toolName = trimmed.slice(HISTORICAL_TOOL_CALL_PREFIX.length, nameClose).trim();
  if (!toolName) {
    return null;
  }

  const jsonStart = nameClose + 1;
  const jsonEnd = consumeBalancedJson(trimmed, jsonStart);
  if (jsonEnd === null) {
    return null;
  }

  const trailing = trimmed.slice(jsonEnd).trim();
  if (trailing.length > 0) {
    return null;
  }

  let argsStart = jsonStart;
  while (argsStart < trimmed.length && /\s/.test(trimmed[argsStart])) {
    argsStart++;
  }

  let args: Record<string, unknown>;
  try {
    args = JSON.parse(trimmed.slice(argsStart, jsonEnd)) as Record<string, unknown>;
  } catch {
    return null;
  }
...
let historicalToolCallIdCounter = 0;

/**
 * Conservative salvage path for models that emit only a fake
 * `[Historical tool call: ...]` thinking block instead of a native tool call.
 *
 * This only upgrades the message when:
 * - there are no real toolCall blocks already present
 * - the message contains at most text preamble plus one historical tool-call thinking block
 * - the thinking content is exactly one parseable historical tool call
 */
export function promoteHistoricalToolCallThinkingToBlocks(message: AssistantMessage): void {
  if (!Array.isArray(message.content) || message.content.length === 0) {
    return;
  }

  let parsed: ParsedHistoricalToolCall | null = null;
  ...
  for (const block of message.content) {
    ...
    if (type === "thinking") {
      ...
      const nextParsed = parseHistoricalToolCallThinking(thinking);
  ...
  if (!sawThinking || !parsed) {
    return;
  }

  const callId = `historical-tool-call-${++historicalToolCallIdCounter}`;
  message.content = [
    {
      type: "toolCall",
      id: callId,
  ...
  // The model stopped normally, but the salvaged content should now execute.
...
export function promoteCallTagsToBlocks(message: AssistantMessage): void {
  ...
  const hasCallTag = message.content.some((b) => {
    ...
    const r = b as unknown as Record<string, unknown>;
    return r.type === "text" && typeof r.text === "string" && r.text.includes(CALL_OPEN);
  });
  if (!hasCallTag) {
    return;
  }
