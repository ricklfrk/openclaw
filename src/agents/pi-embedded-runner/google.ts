...
>(params: {
  ...
  // Google APIs (both Cloud Code Assist and Generative AI) reject JSON Schema
  // keywords such as patternProperties, additionalProperties, $ref, etc.
  // Also sanitize when a Gemini model is accessed through an OpenAI-compatible
  // proxy (modelApi=openai-completions but model name starts with "gemini").
  const isGeminiModel = params.modelId?.toLowerCase().startsWith("gemini") ?? false;
  if (
