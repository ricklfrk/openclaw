  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        // content.  Throw a recognizable error so key-rotation can retry with
        // the next API key (different keys may have different safety thresholds).
        if (output.stopReason === "error") {
          const reason = candidate?.finishReason ?? "unknown";
          if (output.content.length === 0) {
            log.warn(
              `[google-nonstream] safety/content-filter block (finishReason=${reason}); throwing for key rotation retry`,
            );
          }
