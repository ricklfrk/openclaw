...
export async function buildReplyPayloads(params: {
  ...
  const sanitizedPayloads = params.payloads.flatMap((payload) => {
    const sourcePayload = params.isHeartbeat ? sanitizeHeartbeatPayload(payload) : payload;
    let text = sourcePayload.text;

    if (sourcePayload.isError && text && isBunFetchSocketError(text)) {
      text = formatBunFetchSocketError(text);
    }

    if (!text || !text.includes("HEARTBEAT_OK")) {
      return [copyReplyPayloadMetadata(sourcePayload, { ...sourcePayload, text })];
    }
    ...
    return [copyReplyPayloadMetadata(sourcePayload, { ...sourcePayload, text: stripped.text })];
  });
